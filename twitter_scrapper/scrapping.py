...
async def fetch_tweets_by_profile(profile_name, retry_count=0, full_url=None):
    ...
    try:
        ...
        await browser.close()
        return await retry_exception(fetch_tweets_by_profile, profile_name,
                                     retry_count, str(e))
...
async def fetch_trending_hashtags(trending_data, retry_count=0, full_url=None):
    ...
    try:
        ...
        await page.goto("https://x.com/explore/tabs/keyword",
        ...
        print(
            f"trending_topics_elements element is found {trending_topics_elements}"
        )
        ...
        text_content1 = await page.evaluate("(element) => element.textContent",
                                            elements1[0])
        print(f"text_content1 element is found")
        print(
            f"Found {len(trending_topics_elements)} trending topics elements.")
        for index, element in enumerate(trending_topics_elements, start=1):
            ...
            if "Trending" in text:
                ...
                type_trending_match = re.search(r"Trending(.+?)(?=\d|$)",
                                                combined)
                # Extracting posts with optional commas and decimals
                posts_match = re.search(r"([\d,\.]+K?)\s*posts", combined,
                                        re.IGNORECASE)
                item = {
                    "id":
                    data[0],
                    "type":
                    category,
                    "trending": (type_trending_match.group(1).strip()
                                 if type_trending_match else ""),
                    "posts": (posts_match.group(1).strip() +
                              " posts" if posts_match else "0"),
                }
                twitter_data.append(item)
    except (NetworkError, PageError) as e:
        print(f"Error interacting with Twitter: {str(e)}")
        await browser.close()
        return await retry_exception(fetch_trending_hashtags, trending_data,
                                     retry_count, str(e))
    except Exception as e:
        print(f"An unexpected error occurred: {str(e)}")
        await browser.close()
    ...
    try:
        ...
        for post_id in post_ids:
            ...
            twitter_data.append({
                ...
                user_name,
                "tweet_content":
                tweet_content.strip() if tweet_content else "",
                "image_url":
                image_url if image_url else "",
                "reply_count":
                reply_count.strip() if reply_count else "0",
                "like_count":
                like_count.strip() if like_count else "0",
                "repost_count":
                repost_count.strip() if repost_count else "0",
                "bookmark_count":
                bookmark_count.strip() if bookmark_count else "0",
                "timestamp":
        ...
        print(f"Error interacting with Twitter: {str(e)}")
        await browser.close()
        return await retry_exception(scrape_twitter_data_by_post_id, user_name,
